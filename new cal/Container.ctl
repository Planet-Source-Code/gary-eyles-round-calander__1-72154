...
For Each ctrl In UserControl.ContainedControls
    If MaxWidth < (ctrl.Left + ctrl.Width + (m_MarginH)) Then
        MaxWidth = (ctrl.Left + ctrl.Width + (m_MarginH))
    End If
Next
If MaxWidth <> 0 Then
...
Dim ctrl As Object
For Each ctrl In UserControl.ContainedControls
    If MaxHeight < (ctrl.Top + ctrl.Height + (m_MarginV)) Then
        MaxHeight = (ctrl.Top + ctrl.Height + (m_MarginV))
    End If
Next
If MaxHeight <> 0 Then
...
Select Case m_ScrollBars 'Position the controls
    ...
    Case vbBoth     'Both scrolls
        ...
        End With
End Select
    If HScroll.Width >= (400) Then  'More than 400 px
        'HScroll.SmallChange = HScroll.SmallChange * 0.5
    ElseIf HScroll.Width < (400) And HScroll.Width >= (150) Then
        HScroll.SmallChange = HScroll.SmallChange * 2
    Else
        HScroll.SmallChange = HScroll.SmallChange * 4
    End If
    
    If VScroll.Height >= (400) Then   'More than 400 px
        'VScroll.SmallChange = VScroll.SmallChange * 0.5
    ElseIf VScroll.Height < (400) And VScroll.Height >= (150) Then
